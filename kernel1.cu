...
__global__ void k1( float* g_dataA, float* g_dataB, int floatpitch, int width) 
{
    ...
    }else if(threadIdx.x == 1 && i == 1)
    {
        ...
        s_data[threadIdx.x + blockDim.x] = g_dataA[ (i) * floatpitch + (j - 3)]; //W
        s_data[threadIdx.x + (2 * blockDim.x)] = g_dataA[ (i + 1) * floatpitch + (j - 3)]; //SW
        s_data[threadIdx.x] = g_dataA[ (i - 1) * floatpitch + (j - 3)]; //NW
    }else if(threadIdx.x == blockDim.x - 2 && i == width - 2)
    {
