...
__global__ void k1( float* g_dataA, float* g_dataB, int floatpitch, int width) 
{
    ...
    }else if(threadIdx.x == blockDim.x - 1)
    {
        ...
        //grab our next location values and store at the +2 location of shared memory
        s_data[(threadIdx.x) + blockDim.x] = g_dataA[ i * floatpitch + (j - 1)]; //E
        s_data[(threadIdx.x) + (2 * blockDim.x)] = g_dataA[ (i + 1) * floatpitch + (j - 1)]; //SE
        s_data[(threadIdx.x)] = g_dataA[ (i - 1) * floatpitch + (j - 1)]; //NE
